...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    // Show cursor only when inside the container
    if (containerSelector) {
      gsap.set(cursor, { opacity: 0 });
      const container = document.querySelector(containerSelector);
      if (container) {
        const onEnter = () => gsap.to(cursor, { opacity: 1, duration: 0.25, ease: 'power2.out' });
        const onLeave = () => gsap.to(cursor, { opacity: 0, duration: 0.25, ease: 'power2.in' });
        ...
        cursor._containerCleanup = origReturn;
      }
    }
    ...
    const scrollHandler = () => {
      ...
      if (!isStillOver) {
        ...
        // Card moved with scroll — recalculate corner positions from fresh bounding rect
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && cursorRef.current && spinTl.current) {
            const normalizedRotation = gsap.getProperty(cursorRef.current, 'rotation') % 360;
            spinTl.current.kill();
            spinTl.current = gsap
              .timeline({ repeat: -1 })
    ...
    return () => {
      ...
      if (cursor._containerCleanup) cursor._containerCleanup();
    };
